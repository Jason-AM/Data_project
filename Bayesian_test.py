...
#def Phi_poly(input , num_of_params ):
#    Phi = []
#    for i in range( len(input) ):
#        one_data_pt = input[i]
#        row = fit_func_poly(one_data_pt , num_of_params )
#        Phi.append(row)
#    
#    return np.array( Phi )
...
#now define phi itself for polynomial
#PHI = Phi_poly(x_test_lag , poly_order )
PHI = Design_matrix( x_test_lag, fit_func_poly , poly_order )
...
#PHI_new = Phi_poly(x_new_lag , poly_order )
PHI_new = Design_matrix( x_new_lag, fit_func_poly , poly_order )
...
unc_vec_2 = Bayes_unc( PHI , PHI_new , clf.lambda_ , clf.alpha_  )
